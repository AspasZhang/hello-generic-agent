...
6. 掌握 **Goal 模式**的使用，让 GA 围绕一个开放目标持续自驱运转
...
## 6.6 Goal 模式（Goal Mode）

> 给 GA 一个开放目标和时间预算，它会像一个自驱的实习生一样持续工作，直到预算耗尽或目标完成。

前面的自主行动模式适合"有明确 TODO 清单"的场景。但有时候你的需求更开放：

```
你：花 3 小时持续优化我的博客 SEO，能做多少做多少。
```

这种"没有固定终点、持续迭代"的任务，就是 Goal 模式的用武之地。

### 与自主行动的区别

| | 自主行动 | Goal 模式 |
|---|---|---|
| 任务来源 | `TODO.txt` 清单 | 一句话开放目标 |
| 结束条件 | 清单做完 | 时间预算耗尽 |
| 行为模式 | 逐条执行 | 自主规划 → 执行 → 反思 → 继续 |
| 适合场景 | 明确的待办事项 | 探索性、优化类、持续改进类任务 |
...
```
你：进入 Goal 模式，目标是"整理我的 Downloads 文件夹，按类型分类归档"，预算 1 小时。
```
...
GA 会创建一个状态文件 `temp/goal_state.json`：

```json
{
  "objective": "整理我的 Downloads 文件夹，按类型分类归档",
  "budget_seconds": 3600,
...
```bash
GOAL_STATE=temp/goal_state.json python3 agentmain.py --reflect reflect/goal_mode.py
```

| 字段 | 作用 |
|------|------|
| `objective` | 用户原话目标，每轮注入提示词 |
| `budget_seconds` | 时间预算（秒），到期自动收口 |
| `max_turns` | 最大轮次上限，防止死循环 |
| `status` | 状态：`running` → `wrapping_up` → `done_budget` |

</details>

### Goal 模式的运行机制

Goal 模式本质上是一个特殊的反射脚本（`reflect/goal_mode.py`），它的 `check()` 函数每轮都会：

1. **检查预算**：时间或轮次是否耗尽
2. **注入目标提示**：告诉 GA 当前目标、已用时间、剩余时间
3. **自动收口**：预算快到时，提示 GA 进入总结阶段

```text
运行中 → 每轮 check() → 注入目标 + 进度
                ↓ 预算耗尽
         status = wrapping_up → GA 写总结报告
                ↓ 总结完成
         status = done_budget → 停止
```

### 观察进度

Goal 模式运行时，你随时可以查看状态：

```
你：Goal 模式跑到哪了？
```

GA 会读取 `goal_state.json` 告诉你已用轮次和剩余时间。你也可以直接查看文件：

```bash
cat temp/goal_state.json
```

### 停止 Goal 模式

- **自动停止**：预算耗尽后 GA 会自动进入收口轮，写完总结后停止
- **手动停止**：直接终止进程即可

<details>
<summary>💡 适合 Goal 模式的任务类型</summary>

| 类型 | 示例 |
|------|------|
| 持续优化 | "花 2 小时优化项目性能，能快多少快多少" |
| 探索研究 | "花 3 小时调研 RAG 方案，整理对比报告" |
| 批量处理 | "把这 200 张图片都做好压缩和水印，做到预算用完为止" |
| 日常维护 | "没事也找事干，帮我整理代码、修 lint 警告、补注释" |
...
| **Goal 模式** | |
| 状态文件 | `temp/goal_state.json` |
| 反射脚本 | `reflect/goal_mode.py` |
| Goal 模式 SOP | `memory/goal_mode_sop.md` |
...
- **Goal 模式**：开放目标 + 时间预算 → GA 自驱持续工作 → 预算耗尽自动收口
